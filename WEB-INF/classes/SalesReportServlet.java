...
public class SalesReportServlet extends HttpServlet {
  public void doGet(HttpServletRequest request,
	...
	out.println("<section id='content'>");
		...
		if(choice.equalsIgnoreCase("productsold"))
		{
			
		HashMap<String,Integer> noofitemssoldmap = MySQLDataStoreUtilities.getNoOfItemsSold();
		out.println("<table style='width:50%'>");
		out.println("<tr>");
		out.println("<th>Product Name</th>");
		out.println("<th>Price($)</th>");
		out.println("<th>Number of Items sold</th>");
		out.println("<th>Total Sale($)</th>");
		out.println("</tr>");
		
		int totalsold = 0;
		int totalrevenue=0;
		for(String prodname : noofitemssoldmap.keySet())
		{
		double price = MySQLDataStoreUtilities.getPriceFromProdName(prodname);
		totalsold+=noofitemssoldmap.get(prodname);
		totalrevenue+=(price*noofitemssoldmap.get(prodname));
		
		out.println("<tr>");
		out.println("<td>"+prodname+"</td>");
		out.println("<td align='center'>"+price+"</td>");
		out.println("<td align='center'>"+noofitemssoldmap.get(prodname)+"</td>");
		out.println("<td align=center'>"+(price*noofitemssoldmap.get(prodname))+"</td>");
		out.println("</tr>");
		}
		out.println("</table>");
		out.println("<br><br><h3>Total number of items sold : "+totalsold);
		out.println("<br><br><h3>Total Revenue for all items sold :$ "+totalrevenue);
		}
			
		if(choice.equalsIgnoreCase("barchart"))
		{
			response.sendRedirect("BarChartServletSales");
		}
			
		if(choice.equalsIgnoreCase("dailytransaction"))
		{
			TreeMap<String,Integer> getdailytransaction = MySQLDataStoreUtilities.getDailyTransaction();
			out.println("<table style='width:50%'>");
			out.println("<tr>");
		...
		for(String result : getdailytransaction.keySet())
		{
		out.println("<tr>");
		out.println("<td>"+result+"</td>");
		out.println("<td>"+getdailytransaction.get(result)+"</td>");
		out.println("</tr>");
		}
		out.println("</table>");
